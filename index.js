    ...
    if (['.ts', '.tsx'].includes(fileExtname)) {
      ...
      if (apiList) {
        apiList.forEach((item) => {
          ...
          const apiVarList = apiFileContent
            .replace(/const /g, '\nconst ')
            .match(/(?<=const)(.*) = .*?\w['`]/g)
          ...
          apiAuthPath.forEach((apiPath, index) => {
            if (apiNames.includes(apiNameList[index])) {
              let originApiName = (apiPath.match(/\'(.*)?'/) ||
                apiPath.match(/\`(.*)?`/))[1].replace(
                /\${.*?}/g,
                function (match, p1) {
                  return varMap.has(match) ? varMap.get(match) : match
                }
...
const getValidPath = (currentPath, parsePath) => {
  const defaultPathList = ['/index.tsx', '.tsx', '/index.ts', '.ts']

  const currentPathList = parsePath.includes('@')
    ? [currentPath.split('src')[0], parsePath.replace(/@/, 'src')]
  ...
  childrenList?.forEach((path) => {
    if (allData[path]) {
      ...
      if (!allChildrenList.includes(path)) {
        ...
        deepChildren(allData, allData[path].childrenPath, allChildrenList)
      }
    } else {
...
const authAPiDeep = (dataObj) => {
  Object.keys(dataObj).forEach((key) => {
    ...
    const filterRepeat = [
      ...allChildrenApiList,
      ...dataObj[key].authApiList,
    ].reduce((acc, authItem) => {
      const isExist = acc.some(
        (_) => _.method === authItem.method && _.api === authItem.api
      )
